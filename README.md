...
**Approach**  
So, we are resetting start as soon as surplus dips below 0 and setting it to the next index. Lets say the last update to start index was at index k where 0 <= k < n (length of the array);
...
surplus = <code>(gas[k] - cost[k]) + (gas[k + 1] - cost[k + 1]) + ... + (gas[n - 1] - cost[n - 1])</code>  
...
Net Fuel Consumption Before k = <code>(gas[0] - cost[0]) + (gas[1] - cost[1]) + ... + (gas[k - 1] - cost[k - 1])</code>    

Now, totalSurplus contains the total fuel collected from the 0th to the (n-1)th index
total_surplus = <code>(gas[0] - cost[0]) + (gas[1] - cost[1]) + ... (gas[n - 1] - cost[n -1]);</code>  
...
==> <code>(totalSurplus >= 0) ? start : -1</code>  

```javascript
  var canCompleteCircuit = function(gas, cost) {
      let surplus = 0, totalSurplus = 0, start = 0;
      const n = cost.length;
